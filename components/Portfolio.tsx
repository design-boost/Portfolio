                    ...
                    {portfolioData.projects.map((project, idx) => (
                        <div key={idx} className={`portfolio-item ${project.category}`}>
                            <Link href={`/portfolio/${project.slug}`} className="block">
                                <div className="relative overflow-hidden group rounded-2xl border border-black/5 dark:border-white/10 bg-white/70 dark:bg-white/5 aspect-square flex items-center justify-center p-8 after:content-[''] after:absolute after:top-0 after:left-0 after:w-full after:h-full after:bg-gradient-to-t after:from-black/40 after:to-transparent after:opacity-0 after:transition after:ease-out after:duration-[160ms] hover:after:opacity-100">
                                    <Image src={project.mainImage} alt={project.projectTitle} placeholder="blur" className="h-full w-full object-contain transition ease-custom duration-500 group-hover:scale-105 group-hover:blur-[1.4px]" />
                                    <span className="z-[1] absolute top-4 right-4 bg-black/30 px-4 py-2 rounded-full backdrop-blur-[5px] text-white font-mono font-normal text-sm uppercase tracking-[0.5px]">{project.category}</span>
